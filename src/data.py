...
class Data(object):
    ...
    def sliding_window(self, features: list, seq_len: int, j=0):
        ...
        for i in range(0,len(df)-seq_len-1-j,seq_len+1+j):
            ...
            y = df[(i+seq_len):i+seq_len+1+j]["new cases"].to_numpy().flatten()#[0]
            xs.append(x)
            ys.append(y)
    ...
    def create_metaLarrays(self, days_before=None, days_after=None):
        ...
        for district in self.df['District'].unique():
            ...
            if days_after is None and days_before is None:
                X, y = self.sliding_window(district_df, self.previous_days, self.forecast_days-1)
            elif days_after is None and days_before is not None:
                X, y = self.sliding_window(district_df[district_df['Date']<days_before], self.previous_days, self.forecast_days-1)
            elif days_after is not None and days_before is None:
                X, y = self.sliding_window(district_df[district_df['Date']>=days_after], self.previous_days, self.forecast_days-1)
    ...
    def get_mini_dataset(batchSize, iters, num_regions, datasets, test=False):
        ...
        xtrain, ytrain = datasets #Get these datasets from the `create_metaLarrays` function
        ...
        if test:
            ...
            for i in range(num_regions):
                ...
                xtrains.append(xtrain[i][np.arange(0,xtrain[i].shape[0])!=test_idx,:,:])
                ytrains.append(ytrain[i][np.arange(0,xtrain[i].shape[0])!=test_idx,:])
    ...
    def create_dataset(self, traindays, testdays):
        ...
        for district in self.df['District'].unique():
            ...
            xtr, ytr = self.sliding_window(district_df[district_df['Date']==traindays], self.previous_days, self.forecast_days-1)
            xte, yte = self.sliding_window(district_df[district_df['Date']==testdays], self.previous_days, self.forecast_days-1)
